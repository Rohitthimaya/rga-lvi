  ...
  return (
    <AppShell>
      <div className="rounded-2xl border border-[var(--border)] bg-[var(--surface)] p-8 shadow-[var(--shadow)]">
        ...
        <h1 className="mt-4 text-3xl font-semibold tracking-tight">AICanGrow Bot</h1>
        <p className="mt-2 max-w-2xl text-sm leading-relaxed text-[var(--muted)]">
          Upload BC Ministry agriculture documents, inspect chunk quality, and test crop advisory answers with citations.
        </p>
...
function QuickLink(props: { href: string; label: string }) {
  return (
    <Link
      href={props.href}
      className="rounded-xl border border-[var(--border)] bg-[var(--surface-2)] px-4 py-2 text-sm text-[var(--text)] transition-colors duration-200 ease-out hover:border-[var(--action)] hover:bg-[var(--action-soft)]"
    >
      {props.label}
    </Link>
  );
